    ...
    const taskStats = await Task.aggregate([
      {
        $facet: {
          ...
          completedNow: [
            { $match: { status:  TaskStatusEnum.COMPLETED } },
            { $count: "count" },
          ],
          completedPrevious: [
            {
...
export const getTodayTasks = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(500).json({ message: "Failed to load today's tasks" });
  }
};
 
export const getWelcomeSummary = async (req: Request, res: Response) => {
  try {
    const startOfToday = new Date();
    startOfToday.setHours(0, 0, 0, 0);
    ...
    return res.status(200).json(welcomeSummary);
  } catch (error) {
    console.error("getWelcomeSummary error:", error);
